...
|--------|----------|------------|
| spawn | new primitive, not already on set | primitive, color, name, text, position |
| remove | delete object | target (name), or targets for several |
| transform | move/put/place/stack/rotate/scale something already on set | target OR targets, position/rotation/scale, mode (absolute\|relative), transition |
| animate | preset motion | target OR targets, preset (turnaround\|orbit\|bounce), transition |
...
- Plural pronouns ("them", "those", "all of them") → the last group, every member named
- put / place / set / stack / arrange / gather are transform verbs whenever the
  noun is already in the briefing. Spawn only what is genuinely not on set —
  "put the sphere on the pedestal" moves the sphere standing there, it does not
  conjure a second one
