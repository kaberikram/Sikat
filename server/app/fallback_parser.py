...
# Screen-space directions in world axes (Three.js: camera default looks -Z).
...
# section -> (primary param, "more" value, "less" value)
...
# spoken param name -> per-section patch key
...
def parse(text: str, scene: SceneState | None = None) -> list[Intent]:
    ...
    for clause in _CLAUSE_SPLIT.split(text):
        intent = _parse_clause(clause, scene)
...
def _find_scene_target(clause: str, scene: SceneState | None) -> str | None:
    """Resolve a noun in the clause to an actual scene object name."""
    if scene is None:
        return None
    ...
    for obj in scene.objects:
        ...
        # "the sphere" matches CORE_SPHERE; "ball" matches via primitive aliasing
...
def _find_target(clause: str, scene: SceneState | None) -> str | None:
    ...
    if resolved:
        return resolved
    # Pass the bare noun; the client applier substring-matches names.
...
def _find_vec3_after(clause: str, anchor: str) -> tuple[float, float, float] | None:
    m = re.search(
        rf"\b{anchor}\s+{_NUM}[,\s]+{_NUM}[,\s]+{_NUM}", clause
    )
    if not m:
        return None
...
def _parse_clause(clause: str, scene: SceneState | None) -> Intent | None:  # noqa: C901
    ...
    for word, mood in MOOD_WORDS.items():
        if re.search(rf"\b{word}\b", clause) and (
            clause == word
            or re.search(r"\b(mood|scene|vibe|look|feel|feels?|lighting|atmosphere|make it)\b", clause)
        ):
            return Intent(action="set_scene", mood=mood)

    # --- update_fx (before spawn/remove: "add bloom", "remove glitch") ------
    for word, section in FX_WORDS.items():
        if word in clause:
            ...
            if enabled is None and not settings:
                enabled = True  # bare "bloom" means switch it on
            return Intent(
                action="update_fx",
    ...
    if re.search(
        ...
        if touched:
            return intent
        # fall through: "light" mentioned but nothing actionable

    # --- move_camera --------------------------------------------------------
    if re.search(r"\b(camera|cam|viewfinder|shot|zoom|dolly)\b", clause) or re.search(
        r"\b(push in|pull back|pull out)\b", clause
    ):
    ...
    # --- animate presets (before rotate: "spin around", "360") --------------
    if re.search(r"\b(turnaround|turn around|360|spin around|orbit|bounce)\b", clause):
        preset = "turnaround"
        if "orbit" in clause:
            preset = "orbit"
    ...
    # --- remove -------------------------------------------------------------
    if re.search(r"\b(remove|delete|destroy|get rid of)\b", clause):
        target = _find_target(clause, scene)
        if target:
            return Intent(action="remove", target=target)

    # --- set_material (make/paint an EXISTING object a color) ---------------
    if re.search(r"\b(paint|colou?r|tint|make|turn)\b", clause):
        color = _find_color(clause)
        existing = _find_scene_target(clause, scene)
        if color and existing:
    ...
    # --- spawn ---------------------------------------------------------------
    if re.search(r"\b(add|spawn|create|make|drop|place|give me)\b", clause):
    ...
    # --- scale ---------------------------------------------------------------
    if re.search(r"\b(scale|grow|shrink|bigger|smaller|larger|double|halve|half)\b", clause):
        target = _find_target(clause, scene)
        if target:
    ...
    # --- rotate ---------------------------------------------------------------
    if re.search(r"\b(rotate|spin|turn|tilt)\b", clause):
        target = _find_target(clause, scene)
        if target:
    ...
    # --- move -----------------------------------------------------------------
    if re.search(r"\b(move|shift|slide|raise|lower|lift|nudge|bring)\b", clause):
        target = _find_target(clause, scene)
        if target:
