...
async def test_empty_plan_floors_neon_tokyo(monkeypatch, scene):
    ...
    assert any(p.command == "SET_KEYFRAMES" for p in packets)
    assert any(p.command == "PLAYBACK" for p in packets)
...
async def test_open_brief_catalog_animate_recovers_to_keyframes(monkeypatch, scene):
    async def bounce_plan(*args, **kwargs):
        yield llm.Meta(mode="execute", needs_deeper_creativity=False)
        yield llm.Step(
            PlanStep(action="animate", target="CORE_SPHERE", motion="bounce", say="bounce")
        )

    monkeypatch.setattr(llm, "stream_plan", bounce_plan)
    ctx = SessionContext()
    ctx.latest_scene = scene
    token = bind_session(ctx)
    packets: list = []
