...
def _animate_targets(packets) -> list[str]:
    return [
...
def test_every_animate_target_exists_or_is_spawned_first():
    ...
    for scene in (scene_with(), scene_with("CORE_SPHERE", "BOX_01")):
        ...
        for target in _animate_targets(packets):
...
def test_empty_set_spawns_the_hero_before_animating_it():
    ...
    assert _animate_targets(packets) == [HERO_SPAWN_NAME]
...
def test_a_populated_set_is_used_rather_than_conjuring_a_sphere():
    ...
    assert _animate_targets(packets) == ["PEDESTAL"]  # equal scale → first wins
    assert HERO_SPAWN_NAME not in _animate_targets(packets)


def test_still_animates_and_plays():
...
def test_motion_floor_packets_vary_by_command_id():
    ...
    p1 = motion_floor_packets("animate", "cmd-a", scene)
    p2 = motion_floor_packets("animate", "cmd-b", scene)
    a1 = next(p for p in p1 if p.command == "ANIMATE_OBJECT")
    a2 = next(p for p in p2 if p.command == "ANIMATE_OBJECT")
    assert p1 != p2 or a1.payload.motion != a2.payload.motion
