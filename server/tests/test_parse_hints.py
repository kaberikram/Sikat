...
def test_hints_include_staged_marker():
    scene = scene_with("BOX")
    parsed = [("add a red box", parse_one_clause("add a red box", scene))]
    hints = format_parse_hints(parsed, scene, staged_indices={0})
    assert "SCRIPT SUPERVISOR NOTES" in hints
    assert "[already staged]" in hints
    assert "spawn" in hints


def test_hints_empty_when_nothing_parsed():
    hints = format_parse_hints([("xyzzy", None)], None, staged_indices=set())
    assert hints == ""
