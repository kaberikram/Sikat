...
headset would.
    ...
    uv run python scripts/mock_telemetry.py --center-x 2 --center-z -1
"""
...
async def run(
    ...
    async with websockets.connect(url) as ws:
        print(f"connected to {url}; orbiting ({center_x}, {center_z}) for {duration}s")
        t0 = time.time()
        orbit_period = 12.0  # seconds per full circle
        while (t := time.time() - t0) < duration:
            theta = t * (2 * math.pi / orbit_period)
            px = center_x + radius * math.sin(theta)
            pz = center_z + radius * math.cos(theta)
            pose = {
...
def main() -> None:
    ...
    parser.add_argument("--center-x", type=float, default=0.0, help="Stage anchor X")
    parser.add_argument("--center-z", type=float, default=0.0, help="Stage anchor Z")
