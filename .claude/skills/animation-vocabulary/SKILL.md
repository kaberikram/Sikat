...
_This file is the authoritative motion glossary for this repo. A subset of these terms is wired into the Director runtime — see `server/app/motion_vocab.py` (phrase → motion id) and `src/motion-synth.ts` (MotionId inventory); keep those mappings consistent with this glossary when either changes._
