<!-- Keep in sync with ../improve-animations/AUDIT.md — same catalog, two consumers. -->

# Animation Standards

The rule catalog `review-animations` measures a diff against: eight categories, what violates each, and the exact target values to cite in findings. Distilled from Emil Kowalski's design engineering philosophy ([emilkowal.ski](https://emilkowal.ski/)). Never approximate a value that appears here — copy it.
